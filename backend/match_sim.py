...
async def ball_update(matchID):
    ...
    cur = conn.cursor()
    row = cur.execute(
        f"SELECT * FROM matches WHERE matchID = ?;", (matchID,)
    ).fetchone()
    if row:
        info = dict(row)
        if info["matchPlayed"] == -1:
            homeTeamInfo = cur.execute(
                "SELECT name, color FROM teams WHERE teamID = ?;",
                (int(info["homeTeamID"]),),
            ).fetchone()
            homeTeamName = homeTeamInfo["name"]
            homeTeamColor = homeTeamInfo["color"]
            awayTeamInfo = cur.execute(
                "SELECT name, color FROM teams WHERE teamID = ?", (info["awayTeamID"],)
            ).fetchone()
            ...
            if type(info["bFOvers"]) == float:
                ...
                balls_faced = int(info["bFOvers"].split(".")[0]) * 6 + int(
                    info["bFOvers"].split(".")[1]
                )
            ...
            if info["bSOvers"] == "YTB":
                ...
                balls_faced = int(info["bSOvers"].split(".")[0]) * 6 + int(
                    info["bSOvers"].split(".")[1]
                )
        ...
        #     }
            ...
            homeTeamName = cur.execute(
                "SELECT name FROM teams WHERE teamID = ?;", (int(info["homeTeamID"]),)
            ).fetchone()["name"]
            awayTeamName = cur.execute(
                "SELECT name FROM teams WHERE teamID = ?", (info["awayTeamID"],)
            ).fetchone()["name"]
            if info["batFirstID"] == info["homeTeamID"]:
                batFirstName = homeTeamName
                batSecondID = info["awayTeamID"]
...
def get_description(batter, bowler, ball):
    first_line = (
        f"{bowler['fname']} {bowler['lname']} to {batter['fname']} {batter['lname']}\n"
    )
...
def create_live_db(conn, matchID, batTeamID, bfPlayers, bsPlayers):
    ...
    """,
        (
            ...
            bowl2FName,
            bowl2LName,
            json.dumps(fbattingCard),
            json.dumps(fYTB),
            json.dumps(sbowlingCard),
            matchID,
        ),
    )
    conn.commit()
...
def update_log(liveID, conn, label, desc, tag="ball-comm", value=None):
    ...
    existing_log = dict(
        conn.cursor()
        .execute("SELECT log FROM matches WHERE matchID=?", (liveID,))
        .fetchone()
    )["log"]
    ...
    full_log.append(
        json.dumps(
            {"label": label, "value": value, "desc": desc.replace("\n", ""), "tag": tag}
        )
    )
    conn.cursor().execute(
        """
...
def update_live_state(
    ...
    if nsbowler["O"] == 0.0:
        ...
        nsbowlerEcon = nsbowler["RC"] / (
            (6 * int(oversBowled) + int(ballsBowled[0])) / 6
        )
...
def update_scorecard(liveID, conn, batting_team, bowling_team, target):
    ...
    for player in bowling_team:
        try:
            # print(player['O'])
            flOvers = int(player["O"]) + (
                round((player["O"] - int(player["O"])) * 10) / 6
            )
    ...
    if target < 0:
        ...
        """,
            (
                json.dumps(battingCard),
                json.dumps(bowlingCard),
                json.dumps(yet_to_bat),
                liveID,
            ),
        )
        conn.commit()
...
async def simulate_innings(
    liveID, batting_team, batTeamID, bowling_team, bowlTeamID, max_overs=20, target=-1
):
    conn = sqlite3.connect(DB_NAME)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    ...
    while over < max_overs and wickets < 10 and not matchOver:
        ...
        while ball_in_over < 6:
            ...
            if event == "WICKET":
                update_how_out(
                    liveID, conn, desc, batter, sbowler, f"{over}.{ball_in_over-1}"
                )
                wickets += 1
                wickets_this_over += 1
                if wickets < 10:
        ...
        if not any(
            result in over_results for result in ["1", "2", "3", "4", "6", "+", "O"]
        ):
            sbowler["M"] += 1
    ...
    return {"runs": total_runs, "wickets": wickets, "balls": balls, "log": log}
    ...
    for player in all_players:
        ...
        try:
            ...
            if player["RS"] > int(player["highest_score"].replace("*", "")):
                ...
                if (player["howOut"] == "Not Out") and (
                    "*" not in player["highest_score"]
                ):
                    player["highest_score"] = str(player["RS"]) + "*"
                    player["highest_balls_faced"] = player["B"]
                    player["best_bat_ID"] = matchID
...
async def simulate_match(matchID, conn_man):
    ...
    print(
        f"""Season {season_number} Match {match_number}
    ...
    first_innings = await simulate_innings(
        liveID, home_players, homeTeamID, away_players, awayTeamID
    )
    ...
    # time.sleep(INNINGS_PAUSE)
    await asyncio.sleep(INNINGS_PAUSE)
    second_innings = await simulate_innings(
        liveID,
        away_players,
        awayTeamID,
        home_players,
    ...
    if first_innings["runs"] > second_innings["runs"]:
        ...
        cur.execute(
            ...
            """,
            (
                first_innings["balls"] // 6,
                first_innings["runs"],
                second_innings["balls"] // 6,
                second_innings["runs"],
                homeTeamID,
            ),
        )
